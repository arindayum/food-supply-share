...
const PostDetailsPage = () => {
  ...
  useEffect(() => {
    if (user && id) {
      ...
      // if user is not loaded yet but id exists, still try fetching post
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message || 'Failed to claim post.'
      );
    }
  };
  ...
  // };
const onRatingSuccess = (donor) => {
  // donor is the object returned by backend: { _id, rating, ratingCount }
  setHasRated(true);
  if (donor && post) {
    setPost((prev) => {
      if (!prev) return prev;
  ...
  const showRatingForm =
    isClaimer && post.status === 'completed' && !hasRated;

  const ownerRating =
    post.owner?.rating != null ? post.owner.rating.toFixed(1) : 'N/A';
  const ownerRatingCount = post.owner?.ratingCount || 0;
            ...
            <Card.Body className="p-4 p-lg-5 d-flex flex-column h-100">
              <div>
                ...
                <div className="mb-3 text-muted">
                  <span>
                    ...
                    Donated by <strong>{post.owner.name}</strong>
                  </span>
                  <span className="mx-2">|</span>
                  <span>
                    <i className="fas fa-star me-1 text-warning"></i>
                    {ownerRating} ({ownerRatingCount} ratings)
                  </span>
                </div>
                <p className="lead">
                ...
                <ListGroup variant="flush" className="my-4">
                  <ListGroup.Item className="px-0">
                    <i className="fas fa-box-open fa-fw me-2 text-primary"></i>
                    <strong>Quantity:</strong> {post.quantity}
                  </ListGroup.Item>
                  <ListGroup.Item className="px-0">
                    <i className="fas fa-tag fa-fw me-2 text-primary"></i>
                    <strong>Category:</strong> {post.category || 'N/A'}
                  </ListGroup.Item>
                  <ListGroup.Item className="px-0">
                    <i className="fas fa-map-marker-alt fa-fw me-2 text-primary"></i>
                    <strong>Address:</strong> {post.address}
                  </ListGroup.Item>
                  <ListGroup.Item className="px-0">
                    <i className="fas fa-calendar-times fa-fw me-2 text-primary"></i>
              ...
              <div className="mt-auto">
                ...
                {isClaimer && post.status === 'claimed' && (
                  <Alert variant="light" className="text-center">
                    You have claimed this item. Arrange pickup via chat.
                  </Alert>
                )}
                {isClaimer &&
                  ...
                  hasRated && (
                    <Alert variant="success" className="text-center">
                      You have already rated this transaction. Thank you!
                    </Alert>
                  )}
                ...
                <div className="d-grid gap-2">
                  {canClaim && (
                    <Button variant="primary" size="lg" onClick={handleClaim}>
                      Claim Item
                    </Button>
                  )}
                  {canComplete && (
                  ...
                  {canChat && (
                    ...
                    >
                      ...
                      Chat with {isDonor ? 'Claimer' : 'Donor'}
                    </Button>
                  )}
                </div>

                {showRatingForm && (
                  <RateDonorForm postId={id} onRatingSuccess={onRatingSuccess} />
                )}
              </div>
            </Card.Body>
          </Col>
